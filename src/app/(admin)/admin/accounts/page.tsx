  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Deposit Accounts</h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">Manage Crypto USDT accounts. Higher priority = shown more.</p>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          className="bg-amber-500 text-slate-900 px-5 py-2.5 sm:px-6 sm:py-3 rounded-xl font-bold text-sm hover:bg-amber-400 flex items-center gap-2 self-start sm:self-auto">
          <span className="material-symbols-outlined text-[18px]">add</span>{showForm ? "Cancel" : "Add Account"}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="bg-slate-800 border border-slate-700 rounded-2xl p-4 sm:p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1"><label className="text-xs text-slate-400 font-medium uppercase">Method</label>
            <select value={form.method} onChange={(e) => setForm(p => ({ ...p, method: e.target.value }))}
              className="bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white text-sm outline-none">
          ...
          <div className="flex flex-col gap-1"><label className="text-xs text-slate-400 font-medium uppercase">Priority (1-10)</label>
            <input type="number" min="1" max="10" required value={form.priority} onChange={(e) => setForm(p => ({ ...p, priority: e.target.value }))}
              className="bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white text-sm outline-none" /></div>
          <div className="col-span-1 sm:col-span-2">
            <button type="submit" disabled={saving} className="bg-amber-500 text-slate-900 px-8 py-3 rounded-xl font-bold text-sm hover:bg-amber-400 disabled:opacity-50 w-full sm:w-auto">
      ...
      {loading ? <div className="text-slate-400">Loading...</div> : accounts.length === 0 ? (
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 sm:p-12 text-center"><p className="text-slate-400">No accounts yet.</p></div>
      ) : (
        <div className="grid gap-3 sm:gap-4">
          {accounts.map((acc) => (
            <div key={acc.id} className={`bg-slate-800 border rounded-2xl p-4 sm:p-6 transition-colors ${acc.isActive ? "border-slate-700" : "border-red-500/30 opacity-60"}`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="px-2 sm:px-3 py-1 rounded-lg text-[10px] sm:text-xs font-bold uppercase bg-blue-500/20 text-blue-400 flex-shrink-0">{acc.method}</div>
                  <div className="min-w-0"><p className="text-white font-semibold text-sm">{acc.name}</p><p className="text-slate-400 text-xs break-all">{acc.number}</p></div>
                </div>
                <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                  ...
                  <button onClick={() => toggleActive(acc)} className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold ${acc.isActive ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
